...
import frc.lib.W8.util.Timestamped;
...
public class VisionIOPhotonVision implements VisionIO {
  ...
  public void updateInputs(VisionIOInputs inputs, Timestamped<Rotation2d> timestampedHeading) {
    ...
    poseEstimator.addHeadingData(
        timestampedHeading.timestamp().in(Seconds), timestampedHeading.get());
    for (PhotonPipelineResult result : camera.getAllUnreadResults()) {
      ...
      for (PhotonTrackedTarget target : estimate.targetsUsed) {
        totalDistance =
            totalDistance.plus(Meters.of(target.bestCameraToTarget.getTranslation().getNorm()));
    ...
    for (PhotonTrackedTarget target : allTargets) {
      tagObservations.add(
          new TagObservation(
              target.fiducialId, target.getPitch(), target.getYaw(), target.getArea()));
    }
