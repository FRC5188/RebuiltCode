 ...
 *
 * <p>This interface adds methods specific to simulation, allowing WPILib physics simulations to
 * feed data back into the motor IO layer. This creates a realistic simulation where the simulated
...
public interface MotorIOSim extends MotorIO {
   ...
   *
   * <p>This is called by WPILib mechanism simulations (like SingleJointedArmSim) to update the
   ...
   *
   * <p>The rotor is the spinning part inside the motor. This method updates the velocity sensor
   ...
   *
   * <p>This updates the acceleration value based on the physics simulation. Acceleration is useful
   ...
   *
   * <p>This is the first stage of gearing, from the motor's internal rotor to wherever the encoder
   * is mounted. For example, if there's a 2:1 reduction between the motor and encoder, this returns
   * 2.0.
   *
   ...
   *
   * <p>This is the second stage of gearing, from the encoder to the actual mechanism you're
   ...
   *
   * <p>Called when the simulation is shutting down to properly release any resources.
   */
