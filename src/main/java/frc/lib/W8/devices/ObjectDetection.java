...
import frc.lib.W8.io.objectDetection.ObjectDetectionIO;
import frc.lib.W8.io.objectDetection.ObjectDetectionIO.TargetObservation;
import frc.lib.W8.io.objectDetection.ObjectDetectionIOInputsAutoLogged;
...
public class ObjectDetection {
  ...
  public double rangeToTarget_SingleFactorArea(
      TargetObservation targetObservation, float a, float b, float c, float d) {
    return (a * Math.pow(targetObservation.objArea(), 3)
        + b * Math.pow(targetObservation.objArea(), 2)
        + c * targetObservation.objArea()
        + d);
  }

  /**
   * Uses the camera's focal length & trig to estimate range from target; requires no measurement of
   * pitch. Utilizes pinhole model of a camera. Note that camera focal length in pixels = (P * D) /
   * H, where P = perceived width of known object (px), D = known distance from camera (in.), H =
   * known height of object (in.). For unreliable corner detection or object digital height
   ...
   * @param cameraFocalLengthPixels The camera focal length in pixels as determined by a calibration
   *     procedure.
  ...
  public double rangeToTarget_FocalLength(
    ...
    double[] objectDigitalCorners_px = {
      targetObservation.cornerOne()[2],
      targetObservation.cornerTwo()[2],
      targetObservation.cornerThree()[2],
      targetObservation.cornerFour()[2]
    };
    Arrays.sort(objectDigitalCorners_px);
    // Calculate object's digital height in pixels.
    double objectPhysicalHeightPixels =
        objectDigitalCorners_px[objectDigitalCorners_px.length - 1] - objectDigitalCorners_px[0];
    // Return estimated range to object in meters.
    return (cameraCalFactor
        * ((objectPhysicalHeightMeters * cameraFocalLengthPixels) / objectPhysicalHeightPixels));
  }
   ...
   * FOV). This method can produce more stable results than SolvePNP when well tuned, if the full 6d
   * robot pose is not required. Note that this method requires the camera to have 0 roll (not be
   * skewed clockwise or CCW relative to the floor), and for there to exist a height differential
   * between goal and camera. The larger this differential, the more accurate the distance estimate
   * will be. For very small differentials, use rangeToTarget_FocalLength.
   *
   * @param targetObservation A data type containing vision pipeline results for a single target.
   *     Used to determine the pitch & yaw of the target from the centerline of the camera's lens in
   ...
   *     camera's height off the ground, the range offset, installation pitch, and installation yaw.
   * @param targetHeightMeters The physical height of the target off the floor as measured by the
   *     location of the detection reticle in meters. For example, if your detection reticle is set
   *     to the center of the detected object's bounding box, this height should be the elevation
   *     off the ground to the center of the target.
   * @param cameraCalFactor An empirical calibration factor to account for real lens effects (e.g.
   *     blur, distortion, focus).
   ...
   */
  public double rangeToTarget_Pitch(
      TargetObservation targetObservation,
   ...
   * @param cameraCalFactor An empirical calibration factor to account for real lens effects (e.g.
   *     blur, distortion, focus).
   * @param cameraOffset An empirical calibration factor to account for bias in heading estimate as
   ...
   */
  public double headingToTarget_Yaw(
      TargetObservation targetObservation,
      Transform3d cameraTransform,
   ...
   * coordinates. This function effectively transforms the robot's pose by its local dX (range) & dY
   * (heading) to the target; the robot local coodinate system is a frame attached to the robot
   ...
   */
  public Translation2d estimateTargetToField(
   ...
   */
  public void getLastNDetections(
      int N,
      ArrayList<Translation2d> lastNDetections,
    ...
    for (int i = 0; i < lastNDetections.size(); i++) {
      ...
      if ((Math.abs(targetTranslation.getX() - currentTranslation.getX()) <= toleranceMeters)
          && (Math.abs(targetTranslation.getY() - currentTranslation.getY()) <= toleranceMeters)) {
